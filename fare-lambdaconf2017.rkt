...
(define (srad-slide #:sad-issue sad-issue #:sad-question sad-question
                    ...
                    #:has-skad? (has-skad? #f) #:skad? (skad? #f) . body)
  (slide
   (data-transition:
    (cond
   ...
   (do-group-title)
   (when-not (empty? body) (first body))
   (table
    ...
    (map (λ (name sad rad)
         ...
         (t sad-question sad-issue sad-story sad-solution
            (div align: 'right (color "Sad..." #:fg *red*)))
         (t rad-question rad-issue rad-story rad-solution
            (div align: 'right
                 (if skad?
                     (color "Even Better!" #:fg *green*)
                     (color "Better!" #:fg *blue*))))))
   (when-not (empty? body) (rest body))))
...
(slide ()
 ...
 @CB{Reframing from Programs to Programming}
 ~
 ~
 ~
...
(srad-slide
 @h1{The Meta-Story}
 #:sad-question "Programs?" ; (how to...)
 #:sad-issue "Focus on things created"
 #:sad-story '("Static products"
               "Experts before the fact")
 #:sad-solution (t @t{Make "good" choices early}
                   "Impose ignorance")
 #:rad-question "Programming?"
 #:rad-issue "People creating (and using!)"
 #:rad-story (t "Dynamic processes"
                "Interactive learning")
 #:rad-solution (t @t{Ban @em{bad} outcomes early}
                   "Create freedom")
 ~
 @p[class: 'fragment]{
   @br[]
