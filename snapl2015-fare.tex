...
(quite possibly hired by users who care).
}
...
where information isn't the limiting factor,
and instead developers sell services on an open market
that value both their experience and ability to learn new skills, in communities.
}
...
You then see the programmer as grunt worker to whom the \proglang{} is a given.
...
Ask about extending the syntax,
to address hooking of metaprograms into the existing language.
Your story assumes a One True Syntax (as in Common Lisp),
that will be side-effected to accept extensions,
...
\SadBetter{Programming Interfaces}{
...
%\bibitem{Analects} Confucius \textsl{Analects}.
